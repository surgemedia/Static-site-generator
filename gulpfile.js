...
for (var i = all_js.length - 1; i >= 0; i--) {
     if ( all_js[i].indexOf('.scss') > -1 ||  all_js[i].indexOf('.sass') > -1 ||  all_js[i].indexOf('.css') > -1 ) { 
    all_js.splice(i, 1); 

} 
};

var your_js = gulp.src(['cwd/assets/js/*.js']);
for (var i = 0; i < your_js.length; i++) {
    all_js.push(your_js[i]);
};
